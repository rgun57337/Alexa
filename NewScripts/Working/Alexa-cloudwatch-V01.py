...
def aws_cloudwatch_func():
    ...
    session = boto3.session.Session(aws_access_key_id='AKIAJL3T3ANJQEAUKW6A', aws_secret_access_key='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', region_name='us-east-1')
...
def cloudwatch_function(intent, session):
    card_title = intent['name']
    session_attributes = {}
    should_end_session = False

    speech_output = aws_cloudwatch_func()
    reprompt_text = "I did not get the question. Please repeat"
...
def on_intent(intent_request, session):
    ...
    if intent_name == "cloudwatchIntent":
        return cloudwatch_function(intent, session)
